...
if ($request_method == "POST") {
    ...
    {
        $data = $_POST;
        $data['name']     = sanitizeInput($_GET['name']);
        $data['email']    = sanitizeInput($_GET['email']);
        $data['address']  = sanitizeInput($_GET['address']);
        $data['phone']  = sanitizeInput($_GET['phone']);
        $data['regfor']   = sanitizeInput($_GET['regfor']);
        $data['invId']   = sanitizeInput($_GET['invId']);
...
function makePhonepePayment($postData)
{
    try {
        ...
        $callbackUrl = SITE_URL.'/src/payment.php?name='.$postData['name'].'&email='.$postData['email'].'&address='.$postData['address'].'&phone='.$postData['phone'].'&regfor='.$postData['register_for'].'&invId='.$transactionId;
...
function phonePeCallback($data)
{
    ...
    $registerFor    = $data['regfor'];
    

    if($registerFor=='family'){
        $registrationFor = 'Family 360';
    }elseif($registerFor=='you'){
        $registrationFor = 'Speak Alive';
    }else{
        $registrationFor = '';
    }
    ...
    try {
        ...
        if($rData->code=='PAYMENT_SUCCESS'){
            ...
            $variables['transactionId'] = $rData->data->transactionId;
            ...
            $variables['paymentMethod'] = $rData->data->paymentInstrument->type;
            $variables['registeredFor'] = $registrationFor;
...
function formValidation($postData)
{
    ...
    $inputs['amount'] = $postData['amount'];
    $inputs['register_for'] = $postData['register_for'];
